...
function parseNaturalLanguage(message: string): { action: string; target?: string; options?: Record<string, any> } | null {
...
${errorArtifact}`;
    ...
    const report = reportGenerator.generate(
      () => 'AUDIT-1',
      (layer) => findings.filter(f => f.layer === layer),
      auditEngine.getState()
    );
...
async function runVerify(message: string): Promise<string> {
  try {
...
Say: "verify that dist/index.js exists"`;
    ...
    for (const claim of result.claims) {
      ...
      if (claim.proof) response += `  Proof: ${claim.proof}\n`;
      if (claim.error) response += `  Error: ${claim.error}\n`;
      response += `\n`;
    }
...
async function runHookAnalysis(message: string): Promise<string> {
  try {
...
async function runResourceAnalysis(): Promise<string> {
  const findings = state.lastFindings || [];
...
async function runCrossRefAnalysis(): Promise<string> {
  try {
    const target = state.lastAuditTarget || process.cwd();
...
function runFirewallTest(): string {
