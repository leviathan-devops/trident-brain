...
const WHY_EXPLANATIONS: Record<string, string> = {
...
const HOW_FIXES: Record<string, (f: Finding) => string> = {
...
function getVerificationCommand(finding: Finding): string {
...
function extractSemanticContext(targetPath: string): string {
...
`;
  ...
  private describeTarget(targetPath: string): string {
    const parts = targetPath.split('/');
  ...
  private describeFindings(findings: Finding[]): string {
