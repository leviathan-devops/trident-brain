...
import {
  ...
  crossRefVerifier,
  type Finding
} from './algorithmic-core.js';
import { artifactWriter } from './artifact-writer.js';
import { IdentityLoader, formatIdentityHeader, formatIdentityForSystemPrompt } from './identity/index.js';
...
function isTridentAgentFromInput(input: any): boolean {
  const agent = input?.session?.agentName ?? (input?.agent || '');
  return agent === 'trident' || agent.startsWith('trident_');
}

function registerSessionAgent(sessionID: string, agent: string) {
...
function checkToolBlock(input: any): { blocked: boolean; reason?: string; category?: string } {
...
function getWhyProblem(category: string): string {
  switch (category) {
...
function getFixLogic(category: string): string {
  switch (category) {
    case 'SIMULATED_EXECUTION':
...
function getSemanticReportName(targetPath: string): string {
  const now = new Date();
...
function parseIntent(message: string): { action: string; target?: string; options?: Record<string, any> } | null {
...
async function runAudit(target: string, options: Record<string, any> = {}): Promise<string> {
...
function generateDocumentFix(): string {
...
function checkTheatricalBlock(input: any): { blocked: boolean; reason?: string; category?: string } {
...
export default async function TridentBrainPlugin(input: PluginInput): Promise<Hooks> {
  ...
  const hooks: Hooks = {
    ...
    'chat.message': async (input: any, output: any) => {
      ...
      const messagePart = output?.parts?.find((p: any) => p.type === 'text');
...
Identity: Self-aware`;
    ...
    'experimental.chat.system.transform': async (input: any, output: any) => {
      ...
      try {
        ...
        if (output.system) {
          const hasTrident = output.system.some(
            (s: string) => s.includes('TRIDENT BRAIN') || s.includes('Trident Documents')
          );
