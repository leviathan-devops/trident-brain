...
async function resolveIdentityPath(): Promise<string> {
  if (cachedIdentityPath) return cachedIdentityPath;
...
export class IdentityLoader {
  ...
  async loadForRole(role: string): Promise<IdentityBundle> {
    ...
    const files: Record<IdentityFileType, string | null> = {
      'TRIDENT.md': null,
      'IDENTITY.md': null,
      'EXECUTION.md': null,
      'QUALITY.md': null,
    ...
    for (const file of Object.keys(files) as IdentityFileType[]) {
  ...
  private extractMantra(content: string): string {
  ...
  private extractNeverDo(content: string): string[] {
    const match = content.match(/## NEVER Do\n([\s\S]*?)(?=\n##|## )/);
    if (!match) return [];
    return match[1].split('\n').filter(l => l.trim() && l.match(/^\d+\./)).map(l => l.replace(/^\d+\.\s*/, '').trim());
  }

  private extractTheatricalPatterns(content: string): string[] {
