# HOTFIX LOG — Critical Patterns for Kraken Brain Builders (Embedded Edition)
...
**Problem:** If brain init throws, the ENTIRE plugin factory fails and the Kraken orchestrator won't load.
...
**Problem:** Accessing sub-component private fields via `as any` silently breaks when fields are renamed.
...
// In hooks:
tracer.getBufferSize()  // NOT: (tracer as any).buffer?.length
```
...
**Problem:** `opencode run` does NOT fire `tool.execute.before/after` or `experimental.chat.system.transform` hooks.
...
**Problem:** If `"model"` is inside the `"provider"` block, opencode 1.14.x ignores it.

**Fix:** `"model"` must be at the TOP LEVEL of `opencode.json`.
