# BUILD SPEC — Kraken Brain Boilerplate

**Objective:** Build a new Kraken orchestrator brain using the External Edition boilerplate.

## Prerequisites

- bun runtime (v1.3+)
- Access to Kraken Agent baseline (v1.2.9.2+)
- Optional: OpenFang (for external bridge)
...
3. Wire sub-components (store, tracer, sync bridge)
...
### Phase 4: Implement Sync Bridge (if external)
1. Open `src/brains/YOUR_BRAIN/sync-bridge.ts`
2. Define message types
3. Implement send/poll/deliver methods

### Phase 5: Wire Integration
1. Open `src/index.ts`
...
4. Test: sync bridge message round-trip
